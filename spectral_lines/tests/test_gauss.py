...
class TestGauss:
    ...
    def state(self):
        ...
        self.idr_ca = Gauss(idr_spec, line='CaII')
        wave, flux, var = idr_spec.rf_spec()
        self.idr_sim_si = Gauss([wave, flux, var], sim=True)
        self.idr_sim_ca = Gauss([wave, flux, var], line='CaII', sim=True)
        ...
        self.sim_ca = Gauss([sim_wave, sim_flux, sim_var], line='CaII',
                            sim=True)
    ...
    def test_smooth(self, state):
        for name, meas in state.measurements.items():
            ...
            assert np.sum(flux_diff**2/cut_obs_var) < 100
